...
const CompanyDetail = () => {
  const [CompanyDetails, setCompanyDetails] = useState<CompanyDetailInterface | null>(null);
          ...
          <div className="grid gap-10 lg:grid-cols-2 lg:gap-20">
            <div className="flex flex-col gap-4 dark:shadow-gray-800 shadow-lg backdrop-blur-lg rounded-md p-4">
              ...
              <p className="text-gray-500 dark:text-gray-400">
                ...
                Registarion Date: {new Date(CompanyDetails.companyJobDate).toDateString()}
              </p>
              <div className="prose max-w-none">
                {CompanyDetails.companyJobDescription.split('\n').map((line:string, index:number) => (
                    <p key={index} className="text-sm text-gray-500 dark:text-gray-400">
                        {line.split('. ').map((sentence:string, sentenceIndex:number) => (
                            <React.Fragment key={sentenceIndex}>
                                {sentence}
                                {sentenceIndex < line.length - 1 && ' '}
                            </React.Fragment>
                        ))}
                        {index < CompanyDetails.companyJobDescription.split('\n').length - 1 && <br />}
                    </p>
                ))}          </div>
